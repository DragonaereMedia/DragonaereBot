...
class InteractionHandler extends EventEmitter {
    ...
    build() {
        ...
        const directories = readdirSync(`${this.client.location}/commands/music/interactions`, { withFileTypes: true });
        for (const directory of directories) {
            if (!directory.isDirectory()) continue;
            const commands = readdirSync(`${this.client.location}/commands/music/interactions/${directory.name}`, { withFileTypes: true });
            for (const command of commands) {
                if (!command.isFile()) continue;
                const Interaction = require(`${this.client.location}/commands/music/interactions/${directory.name}/${command.name}`);
